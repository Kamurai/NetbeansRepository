...
public class ManagerBean
{
    ...
    public String updatecustomers()
    {
            ...
            if( newcustomerinfo.get(x).getFirstname().compareTo("") != 0 || newcustomerinfo.get(x).getLastname().compareTo("") != 0 || newcustomerinfo.get(x).getEmail().compareTo("") != 0)
            {
                ...
                for(int y = 0; y < customerinfo.size(); y++)
                {
                    ...
                    if(customerinfo.get(y).getCustomerid() == cid)
                    {
                            ...
                            if(newcustomerinfo.get(x).getEmail().compareTo("") != 0  && validateemail(newcustomerinfo.get(x).getEmail()))
                            {
        ...
        for(int x = 0; x < newusernames.size(); x++)
        {
            ...
            if (newusernames.get(x).getUsername().compareTo("") != 0 || newusernames.get(x).getPassword().compareTo("") != 0 || (newusernames.get(x).getManager() == true) );
            {
                ...
                if (newusernames.get(x).getUsername().compareTo("") != 0 && clearusername(newusernames.get(x).getUsername()))
                {
                    //build sql statement to update based on customerid: sqlstatement += " FIRSTNAME = '" + firstname + "'";
    ...
    {
        if (theEmail.compareTo("") != 0)
        {
            // Check at-sign and white-space usage
            int atSign = theEmail.indexOf('@');
            if (atSign == -1
                || atSign ==0
                || atSign == theEmail.length() - 1
                || theEmail.indexOf('@', atSign + 1) != -1
                || theEmail.indexOf(' ') != -1
                || theEmail.indexOf('\t') != -1
                || theEmail.indexOf('\n') != -1
                || theEmail.indexOf('\r') != -1
                )
            {
                return false;
            }
            ...
            int dot = theEmail.indexOf('.');
            if (dot == -1
                    || dot == 0
                    || dot == theEmail.length() - 1)
            {
                return false;
            }
    ...
    public boolean clearusername(String theName)
    {
