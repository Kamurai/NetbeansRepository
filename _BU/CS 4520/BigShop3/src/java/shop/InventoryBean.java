...
public class InventoryBean
{
    ...
    public void createtable(String table)
    {
        try
        {
            ...
            tables.add(table);
            db.writeTo("INSERT INTO BIGSHOP.TABLENAMES (TABLENAME) VALUES ('" + table + "')");
        }
        catch(Exception ex)
        {
            ex.printStackTrace();
    ...
    public void updateproducts()
    {
        ...
        for(int x = 0; x < newproducts.size(); x++)
        {
            ...
            if( newproducts.get(x).getPicture().compareTo("") != 0 || newproducts.get(x).getName().compareTo("") != 0 || newproducts.get(x).getPrice().compareTo("") != 0 || newproducts.get(x).getInventory().compareTo("") != 0 || newproducts.get(x).getTable().compareTo("") != 0 || newproducts.get(x).getDescription().compareTo("") != 0)
            {
                //determine if current table does not exist
                if(!determinetable(newproducts.get(x).getTable()))
                {
                    //create table
                    createtable(newproducts.get(x).getTable());
                }
    ...
    public String createproduct()
    {
        ...
        //validate first
            //productname
        if(!validateproductname(theTable))
        {
    ...
    public boolean validateproductname(String theTable)
    {
        boolean trip = true;
            ...
            if(rs != null)
            {
                try
                {
                    ...
                    while( rs.next() )
                    {
                        ...
                        if( productname.compareTo(test) == 0 )
                        {
                            trip = false;
                            rs.last();
                        }
