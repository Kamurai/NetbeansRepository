...
public class ShopBean
{
    ...
    public String checkout()
    {
        ...
        //set orderbean's total
        ob.setTotal(ib.getTotal());
    ...
    public boolean isOrderResolved()
    {
        ...
        //store purchase receipt
            ...
            {
                //if more than 0 of a product is requested
                if(Integer.parseInt(ib.info.get(x).getAmount()) > 0 )
                {
                    //record the purchase
                    ob.purchaselist.add(new PurchaseBean(ob.getRecordid(), db.getnextavailableid("PURCHASES", "PURCHASEID"), ib.info.get(x).getName(), ib.info.get(x).getPrice(), Integer.parseInt(ib.info.get(x).getAmount()), ib.info.get(x).getSubtotal(), ib.info.get(x).getDescription()));

                    //write to database
                    sqlstatement = "INSERT INTO BIGSHOP.PURCHASES(PURCHASEID, RECORDID, PRODUCTNAME, PRODUCTPRICE, PRODUCTAMOUNT, PRODUCTSUBTOTAL, DESCRIPTION) VALUES( " + db.getnextavailableid("PURCHASES", "PURCHASEID") + ", " + ob.getRecordid() + ", '" + ib.info.get(x).getName() + "', " + ib.mod(ib.info.get(x).getPrice()) + ", " + Integer.parseInt(ib.info.get(x).getAmount()) + ", " + ib.mod(ib.info.get(x).getSubtotal()) + ", '" + ib.info.get(x).getDescription() + "')";
        ...
        for(int x = 0; x < ib.info.size(); x++)
        {
            ...
            //if product amount is equal to 0
                if( Integer.parseInt(ib.info.get(x).getInventory()) == 0 )
                {
                    //mark deletions
                    ib.deleteproduct.set(x, true);
                }
        }
