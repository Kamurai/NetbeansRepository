...
public class OrderBean implements Serializable
{
    ...
    public void buildreceipts()
    {
        ...
        try
        {
            ...
            while (rs.next())
            {
                ...
                purchaselist.add(new PurchaseBean(getRecordid(), getPurchaseid(), getProductname(), getProductprice(), getProductamount(), getProductsubtotal(), getProductdescription()));
            }
        }
        catch (Exception ex)
        {
    ...
    public boolean isOrderGood()
    {
        ...
        //validate information
            //postal code
        if( !validatenumwithhyphen(getRpcode()))
        {
            setError("Recipient postal code error");
            return false;
        }
        if( !validatenumwithhyphen(getBpcode()))
        {
            setError("Billing postal code error");
            return false;
        }
        //phone number
        if( !validatenumwithhyphen(getRphone()))
        {
            setError("Recipient phone number error");
            return false;
        }
        if( !validatenumwithhyphen(getBphone()))
        {
            setError("Billing phone number error");
            return false;
        }
        //credit card number
        if( !validatenum(getCcnumber()))
        {
            setError("Credit card number error");
            return false;
        }
        //credit card exp
        if( !validatenumwithslash(getCcexp()))
        {
            setError("Credit card expiration error");
            return false;
        }
        //credit card security code
        if( !validatenum(getCccode()))
        {
            setError("Credit card security code error");
            return false;
        }

        //set Date Of Purchase
        //setDop(Integer.toString(Calendar.DATE) + " " + Integer.toString(Calendar.MONTH) + " " + Integer.toString(Calendar.YEAR));
        ...
        //set recordid
        setRecordid(db.getnextavailableid("Records", "RECORDID"));

        //add record to arraylist
        recordlist.add(new RecordBean(getRecordid(), getUsername(), getEmail(), getRfname(), getRlname(), getRstreet(), getRcity(), getRstate(), getRcountry(), getRpcode(), getRphone(), getBfname(), getBlname(), getBstreet(), getBcity(), getBstate(), getBcountry(), getBpcode(), getBphone(), getCctype(), getCcnumber(), getCcexp(), getCccode(), getDop(), getTotal()));

        //write record to database
        String sqlstatement = "INSERT INTO BIGSHOP.RECORDS (recordid, username, email, rfname, rlname, rstreet, rcity, rstate, rcountry, rpcode, rphone, bfname, blname, bstreet, bcity, bstate, bcountry, bpcode, bphone, cctype, ccnumber, ccexp, cccode, dop, total) ";
        sqlstatement += "VALUES (" + getRecordid() + ", '" + getUsername() + "', '" + getEmail() + "', '" + getRfname() + "', '" + getRlname() + "', '" + getRstreet() + "', '" + getRcity() + "', '" + getRstate() + "', '" + getRcountry() + "', '" + getRpcode() + "', '" + getRphone() + "', '" + getBfname() + "', '" + getBlname() + "', '" + getBstreet() + "', '" + getBcity() + "', '" + getBstate() + "', '" + getBcountry() + "', '" + getBpcode() + "', '" + getBphone() + "', '" + getCctype() + "', '" + getCcnumber() + "', '" + getCcexp() + "', '" + getCccode() + "', '" + getDop() + "', '" + Double.toString(mod(getTotal())) +"')";
    ...
    public boolean validatenumwithhyphen(String target)
    {
        boolean flag = true;
    ...
    public boolean validatenumwithslash(String target)
    {
        boolean flag = true;
    ...
    public boolean validatenum(String target)
    {
        boolean flag = true;
    ...
    public int getpurchasesize(int input)
    {
        int num = 0;
        ...
        {
            //walkthrough recordlist
            //for(int y = 0; y < recordlist.size(); y++)
            //{
                //if recordid matches current recordid
                if( purchaselist.get(x).getRecordid() == recordlist.get(input).getRecordid() )
                {
                    //increment counter
                    num++;
                }
    ...
    public String fetchpurchase(int index, int num, int property)
    {
        //walkthrough purchases
        for(int x =0; x < purchaselist.size(); x++)
        {
            //if recordid matches current recordid
            if( purchaselist.get(x).getRecordid() == recordlist.get(index).getRecordid() )
            {
                if( num == 0 )
                {
                    if( property == 1 )
                    {
                        return purchaselist.get(x).getProductname();
                    }
                    else if( property == 2 )
                    {
                        return purchaselist.get(x).getProductprice();
                    }
                    else if( property == 3 )
                    {
                        return Integer.toString(purchaselist.get(x).getProductamount());
                    }
                    else if( property == 4 )
                    {
                        return purchaselist.get(x).getProductsubtotal();
                    }
                    else if( property == 5 )
                    {
                        return purchaselist.get(x).getProductdescription();
                    }
                }
                else
                {
